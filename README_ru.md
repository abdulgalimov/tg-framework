...
// 4. Создайте сервисы (подключите store, kv, locale)
tg.create({
  store: myStore,
  kv: myKvStore,
  locale: {
...
// 5. Инициализируйте и запустите polling
await tg.init(async () => {
...
Файлы локализации используют формат `key=value` с интерполяцией `${arg}`:
